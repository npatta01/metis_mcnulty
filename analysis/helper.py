...
def plot_confusion_matrix(model,actual_classes,features):
   y_predicted=model.predict(features)
   label_names = model.classes_
   print ("Accuracy Score: %s"%(accuracy_score(y_test, y_predicted))
   cm = confusion_matrix(actual_classes, y_predicted)
